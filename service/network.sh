...
EOF"
	...
	ssh root@$TARGET ". adminrc;openstack user create --domain default --password $PASSWD neutron"
	
	ssh root@$TARGET ". adminrc;openstack role add --project service --user neutron admin"
	ssh root@$TARGET ". adminrc;openstack service create --name neutron --description \"OpenStack Networking\" network"

	ssh root@$TARGET ". adminrc;openstack endpoint create --region RegionOne network public http://$TARGET:9696"
	ssh root@$TARGET ". adminrc;openstack endpoint create --region RegionOne network internal http://$TARGET:9696"
