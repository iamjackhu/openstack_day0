...
EOF"
	...
	ssh root@$TARGET "openstack user create --domain default --password-prompt glance<<EOF
$PASSWD
$PASSWD
EOF"

	ssh root@$TARGET "openstack role add --project service --user glance admin"
	ssh root@$TARGET "openstack service create --name glance --description \"OpenStack Image\" image"

	ssh root@$TARGET "openstack endpoint create --region RegionOne image public http://$TARGET:9292"
	ssh root@$TARGET "openstack endpoint create --region RegionOne image internal http://$TARGET:9292"
	ssh root@$TARGET "openstack endpoint create --region RegionOne image admin http://$TARGET:9292"
